...
namespace Ohana3DS_Rebirth.Ohana
{
    public class ETC1
    {
            ...
            for (int y3 = 0; y3 < 4; y3++)
            {
                for (int x3 = 0; x3 < 4; x3++)
                {
                    ...
                    if ((flipbit && y3 < 2) || (!flipbit && x3 < 2))
                    {
                        ...
                        c = ToColorFormat((byte)(((Alpha >> ((x3 * 4 + y3) * 4)) & 0xF) * 0x11), (byte)ColorClamp(r1 + add), (byte)ColorClamp(g1 + add), (byte)ColorClamp(b1 + add));
                    }
                    else
                    {
                        int add = ETC1Modifiers[Table2, val] * (neg ? -1 : 1);
                        c = ToColorFormat((byte)(((Alpha >> ((x3 * 4 + y3) * 4)) & 0xF) * 0x11), (byte)ColorClamp(r2 + add), (byte)ColorClamp(g2 + add), (byte)ColorClamp(b2 + add));
                    }
        ...
        public static uint ToColorFormat(uint A, uint R, uint G, uint B)
        {
            uint result = 0;
            uint mask;
            mask = ~(0xFFFFFFFFu << 8);
            result |= ((A * mask + 127u) / 255u) << 24;
            mask = ~(0xFFFFFFFFu << 8);
            result |= ((R * mask + 127u) / 255u) << 16;
            mask = ~(0xFFFFFFFFu << 8);
            result |= ((G * mask + 127u) / 255u) << 8;
            mask = ~(0xFFFFFFFFu << 8);
